...
void nn_mat_mul(float* inputs, float* weights, float* outputs, int num_samples, int input_size, int output_size) {
    dim3 threadsPerBlock(output_size, num_samples);
    std::cout << "threadsPerBlock: " << threadsPerBlock.x << " " << threadsPerBlock.y << std::endl;
    dim3 blocksPerGrid(1, 1);
        if (output_size*num_samples > 512){
            threadsPerBlock.x = 512;
            threadsPerBlock.y = 512;
            blocksPerGrid.x = ceil(output_size/double(threadsPerBlock.x));
            blocksPerGrid.y = ceil(double(num_samples)/double(threadsPerBlock.y));
        }
    nn_matrix_multiplication_kernel<<<blocksPerGrid,threadsPerBlock>>>(inputs, weights, outputs, num_samples, input_size, output_size);
}
